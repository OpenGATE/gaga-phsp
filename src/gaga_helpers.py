...
def project_on_plane(x, plane, image_plane_size_mm, debug=False):
    ...
    ## Point (1-4) and direction (4-7) (generated by the gan)
    p = x[:, 1:4]
    u = x[:, 4:7]

    # print('points:', p[debug_id])
    # print('dir:', u[debug_id])
    pid = np.arange(len(p))
    #print(pid)
    ...
    ## vector from all points to the plane center
    #w = c0 - p
    w = p - c0  ## ray-point minus plane-point (center)
    print('c0', c0[0])
    #print('w', w[debug_id])
    ...
    print('DEBUG ndotu = ', ndotu[debug_id])
    print('DEBUG si = ', si[debug_id])
    
    # only positive (direction to the plane, not opposite ?) FIXME to remove when several planes
    ...
    debug_newid = np.where(mpid == debug_id)[0]
    print('debug_newid', debug_newid)

    print('DEBUG Psi = mp + msi*mu     mpid = ', mpid[debug_newid])
    print('DEBUG Psi = mp + msi*mu     mp   = ', mp[debug_newid])
    print('DEBUG Psi = mp + msi*mu     msi  = ', msi[debug_newid])
    print('DEBUG Psi = mp + msi*mu     mu   = ', mu[debug_newid])
    print('DEBUG Psi = mp + msi*mu     psi  = ', psi[debug_newid])
    
    print('point 3D initial   = ', mp[debug_id])
    print('dir   3D initial   = ', mu[debug_id])
    ...
    # FIXME 
    # psip = r.apply(psi) - r.apply(mc0) ## plane rotation and translation
    # psip = r.apply(mc0) - r.apply(psi) ## plane rotation and translation
    # psip = r.apply(psi)
    ri = r.inv()
    # print('mc0', mc0[0])
    # print('r mc0', ri.apply(mc0)[0])
    # #offset = [image_plane_size_mm[0]/2.0, image_plane_size_mm[1]/2.0, 0]
    # offset = [image_plane_size_mm[0]/2.0, 0, 0]
    # print(offset)
    # offset = np.array([offset]*len(psi))
    psip = ri.apply(psi) #- offset
    #  + ri.apply(mc0)
    
    print('DEBUG Psip with rotation = ', psip[debug_newid])
    #print('psip', psip)
    ...
    idx = np.where(mpid == debug_id)[0]
    print('idx', idx)
    idx = np.where(mmpid == debug_id)[0]
    print('idx', idx)
    print('point ->', psip[idx])
    print('dir before ->', mu[idx])
    ...
    dx = mup[:,0]
    dy = mup[:,1]
    print('dir after ->', mup[idx])
    print('dir acos r->', np.arccos(mup[idx]))
    print('dir acos d->', np.rad2deg(np.arccos(mup[idx]))) ## <-- OK !!
    ...
    print('mmpid', mmpid.shape)
    mmpid = mmpid.reshape((nb,1))
    print('mmpid', mmpid.shape)
    data = np.concatenate((data, mmpid), axis=1) # v
...
def gaga_garf_generate_image(p):
    ...
    while ev<n:
        ...
        if debug_p: # FIXME DEBUG
            x = debug_p.events_data
            print('Reading x from events file', len(x))
        else:
        ...
        if debug_p:
            debug_compare_planar_points(debug_p, px)
